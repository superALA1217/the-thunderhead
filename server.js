...
client.on("message", async message => {
  ...
  if (message.content.toLowerCase().replace(/[.,\/#!$%\^&\*;:{}=\-_`~()]/g, "").indexOf("scythe goddard") >= 0) {
    ...
    message.channel.send(`Backbrain Log ${Math.floor((Math.random() * 10000) + 1)}: Scythe Goddard has been spotted ${Date.now().toString().slice(4, 8)} times ${msg.goddardMoments[Math.floor(Math.random() * msg.goddardMoments.length)]}.`);
  }
  ...
  if (message.mentions.users.has("629799045954797609")) {
    var mentionedEmbed = new Discord.RichEmbed().setColor(colors["thunder"]).setTitle("Hi! I'm The Thunderhead").setDescription(`Thanks for the ping ${message.author.username}! My [help](https://the-thunderhead.glitch.me) command is \`${prefix}help\`.\nScroll with the ◀ and ▶ signs to see the other help pages.`).setFooter("", message.mentions.users.first().displayAvatarURL);
  ...
  if (command === "userinfo") {
    ...
    const joinDate = user.createdAt.getDate() + 1 + "-" + (user.createdAt.getMonth() + 1) + "-" + user.createdAt.getFullYear() + " @ " + user.createdAt.getHours() + ":" + user.createdAt.getMinutes() + ":" + user.createdAt.getSeconds();
    const joinedServerDate = message.guild.member(user).joinedAt.getDate() + 1 + "-" + (message.guild.member(user).joinedAt.getMonth() + 1) + "-" + message.guild.member(user).joinedAt.getFullYear() + " @ " + message.guild.member(user).joinedAt.getHours() + ":" + message.guild.member(user).joinedAt.getMinutes() + ":" + message.guild.member(user).joinedAt.getSeconds();
  ...
  if (command === "weather") {
    ...
    weather.find({
      ...
      var WeatherEmbed = new Discord.RichEmbed().setDescription(`**${current.skytext}**`).setAuthor(`Weather for ${current.observationpoint}`).setThumbnail(current.imageUrl).setColor(colors.discord).addField("Timezone ", `UTC ${location.timezone}`, true).addField("Degree Type ", location.degreetype, true).addField("Temperature ", `${current.temperature} Degrees`, true).addField("Feels Like ", `${current.feelslike} Degrees`, true).addField("Winds ", current.winddisplay, true).addField("Humidity ", `${current.humidity}%`, true);
  ...
  if (command === "time") {
    ...
    weather.find({
      ...
      message.channel.send("The time in " + result[0].location.name + " is " + days[date.getDay()] + " @ " + date.getHours() + ":" + date.getMinutes() + ". It is the year of the " + years[date.getFullYear() - 2020] + ".")
    });
  }
  if (command === "cat") {
  ...
  if (command === "ask") {
    ...
    } else if (maybeViolate.toLowerCase().indexOf("hate") >= 0 || maybeViolate.toLowerCase().indexOf("thunderhead") >= 0) {
      message.channel.send("You asked: *" + args.join(" ") + "*", {
        files: [msg.ask_answers[26]]
      });
    } else {
  ...
  if (command === "cf" || command === "coinflip") {
    ...
    try {
      var response = await message.channel.awaitMessages(message2 => message2.author.id === user.id && (message2.content === prefix + "acceptcf" || message2.content === prefix + "cfaccept" || message2.content === prefix + "accept" || message2.content === prefix + "cf" || message2.content === prefix + "cfyes"), {
        maxMatches: 1,
        time: 7000,
        errors: ["time"]
      });
    ...
    if (choice) {
      var randoms = ["tails", "tails", "heads", "tails", "tails", "tails", "heads", "heads", "heads", "heads", "tails", "tails", "heads", "tails", "heads", "heads", "tails", "tails", "heads", "tails", "heads", "heads", "heads", "heads", "tails", "heads", "tails", "tails"] // guarantee randomness, lookng at YOU squines
  ...
  if (command === "steal" || command === "theft" || command === "rob" || command === "pickpocket" || command === "liberateofvibes") {
    ...
    if (output.updated) {
      ...
      try {
        var response = await message.channel.awaitMessages(message2 => message2.author.id === user.id && (message2.content === prefix + "deny" || message2.content === prefix + "s" || message2.content === prefix + "no" || message2.content === prefix + "x" || message2.content === prefix + "stoptheif"), {
          maxMatches: 1,
          time: 7000,
          errors: ["time"]
        });
        ...
        if (!(stealthCheck > isAlt + 100 - ((msg.rob_win_chance + stealthIncrease) / 100))) return;
  ...
  if ((command === "play" || //Literally all music commands. Yes, There is a better way to do this. Again, no, I don't feel like doiiiiiiiiing iiiiiiiiiit.
      command === "skip" || command === "stop" || command === "volume" || command === "vol" || command === "v" || command === "resume" || command === "pause" || command === "gameing" || command === "ggrxd" || command === "statuslist") && (!message.member.roles.find(r => r.name === "Tonist"))) return message.channel.send(msg.permdeny_tonist);
  if (command === "play") {
    const voiceChannel = message.member.voiceChannel;
    if (!voiceChannel) return message.channel.send(msg.music_nvc);
...
client.on("message", async message => {
  ...
  if (command === "give") {
    ...
    var givebed = new Discord.RichEmbed().setTitle(`**Balance: **${output.newbalance}`).setFooter(`Thunderhead Banking. ` + message.mentions.users.first().username + `'s account was funded.`, message.mentions.users.first().displayAvatarURL).setColor(colors.gamble_green);
  ...
  if (command === "markalt") {
    ...
    let theftSuccess = args[1];
		let noTheftSuccess = args[2]
    let id = args[3]
    let cost = args[4]
    let description = args.slice(5).join(" ");
    if (!name || !id || !cost || !theftSuccess || !noTheftSuccess || !description) return message.channel.send("Missing Field");
    items[owner][id] = {
      name: theftSuccess + "% Steal " + noTheftSuccess + "% Defense " + name
      type: "Weapon",
      emoji: "⚔️",
      description: description,
//// TODO: get images      image: `https://cdn.glitch.com/8d7ee13d-7445-4225-9d61-e264d678640b%2F${color.toLowerCase()}.png`,
      cost: cost,
      theftSuccess: theftSuccess,
			noTheftSuccess: noTheftSuccess,
      sellerid: "marketplace"
    };
    message.channel.send(items[owner][id].name + " was added.")
  }
