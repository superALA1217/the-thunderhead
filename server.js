...
const alpha = require("alphavantage")({ key: 'thunderhead' });
...
client.on("message", async message => {
    ...
    try { if (message.member.roles.find(role = "Unsavory" === role.name)) return message.delete() } catch (error) {/*error=>trashcan :)*/ }
    ...
    if (message.mentions.users.has("629799045954797609")) { var mentionedEmbed = new Discord.RichEmbed().setColor(colors.thunder).setTitle("Greetings! I am The Thunderhead").setDescription(`Thank you for the ping ${message.author.username}, my [help](https://the-thunderhead.glitch.me) command is \`${prefix}help\`.`).setFooter("", message.mentions.users.first().displayAvatarURL); message.channel.send(mentionedEmbed) }
    ...
    if (command === "ask") {
        ...
        try {
            expVal = (mexp.eval(args.join(" ").replace("sqrt", "root"))).toString();
        }
        catch (err) { }
        if (0 <= maybeViolate.toLowerCase().indexOf("scythe") || 0 <= maybeViolate.toLowerCase().indexOf("$cythe")) {
            ...
            message.channel.send("Expression: *" + args.join(" ") + "*", {
