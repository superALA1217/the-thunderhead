...
app.get("/top", function (request, response) {response.sendFile(__dirname + "/views/leaderboard.html");});
...
// JSON
const helpList = require("./help.json"); //READ ONLY
const msg = require("./msgs.json"); //READ ONLY
const reminds = require("./reminds.json"); //WRITE 
const events = require("./events.json"); //WRITE 
...
const currency = `vibes`  //Todo: Implement Vibe Emoji
...
const itemTypeKey = {
    ...
    "general": { "name": "", "emoji": "📜" },
    "eco": { "name": "Economy ", "emoji": currency },
    "music": { "name": "Music ", "emoji": "🎵" },
    "mod": { "name": "Moderation & Utility ", "emoji": "🔧" }
};
const colors = {
...
var repoImageDatabase = "https://raw.githubusercontent.com/humboldt123/shameless_image_dump/master/thunderTXT/"; //yes this IS a repo i use to host the images, yes i KNOW i there are better options, no i dont FEEL like it
...
client.on("message", async message => {
    ...
    if (message.content.toLowerCase().replace(/[.,\/#!$%\^&\*;:{}=\-_`~()]/g, "").indexOf("scythe goddard") >= 0) {
        ...
        items = ["taking pennies from the donation box",
        ...
        message.channel.send(`Backbrain Log 343: Scythe Goddard has been spotted ${Date.now().toString().slice(4, 8)} times ${items[Math.floor(Math.random() * items.length)]}.`);
    }
    ...
    if (message.mentions.users.has("629799045954797609")) {
        var mentionedEmbed = new Discord.RichEmbed()
            ...
            .setDescription(`Thanks for the ping ${message.author.username}! My [help](https://the-thunderhead.glitch.me) command is \`${prefix}help\`.\nScroll with the ◀ and ▶ signs to see the other help pages.`)
    ...
    if (command === "help") {
        ...
        if (!helpText) {
            ...
            for (var itemType in helpList) {
                if (commandTypeIndex > 0) { for (item in helpList[itemType]) { if (helpList[itemType][item]["name"] === args[0]) commandData = helpList[itemType][item]; } } commandTypeIndex++;
            }
    ...
    if ((command === "event") || (command === "newevent") || (command === "createevent") || (command == "e")) {
        if (!(events[message.channel.id])) { events[message.channel.id] = [] }
        var date = new Date();
        var time = args[0]
        ...
        newtime = (date.getTime() + (unit * 1000 * time))
        date = date.getTime() + (unit * 0)
        events[message.channel.id].push({ "reminder": themessage, "time": newtime })
    ...
    if (command === "ask") {
        ...
        if (maybeViolate.toLowerCase().indexOf("scythe") >= 0 || maybeViolate.toLowerCase().indexOf("$cythe") >= 0) { message.channel.send("You asked: *" + args.join(" ") + "*", { files: [repoImageDatabase + "warning.jpg"] }); }
        else if (maybeViolate.toLowerCase().indexOf("hate") >= 0 || maybeViolate.toLowerCase().indexOf("thunderhead") >= 0) { message.channel.send("You asked: *" + args.join(" ") + "*", { files: [repoImageDatabase + "non-scythes.jpg"] }); }
        else {
            var scytheRandom = Math.floor(Math.random() * thunderAnswer.length);
            message.channel.send("You asked: *" + args.join(" ") + "*", { files: [repoImageDatabase + thunderAnswer[scytheRandom]] });
        }
    }
    ...
    if (command === "roll") {
        ...
        if (isNaN(stake) && (function (x) { return (x | 0) === x; })(parseFloat(stake))) return message.channel.send(msg.roll_validint);
    ...
    if (command === "cf" || command === "coinflip") {
        ...
        try {
            var response = await message.channel.awaitMessages(
                message2 => message2.author.id === user.id && (message2.content === prefix + "acceptcf" || message2.content === prefix + "cfaccept" || message2.content === prefix + "accept" || message2.content === prefix + "cf" || message2.content === prefix + "cfyes"), {
                    maxMatches: 1,
                    time: 7000,
                    errors: ["time"]
                }
    ...
    **Now playing:** ${serverQueue.songs[0].title}
    ...
    if (command == "poll") {
        ...
        if (!args[0]) return message.channel.send(msg.poll_novalidq).then(message.channel.bulkDelete(1)).then(msg => msg.delete({ timeout: 1000000000000 }));
